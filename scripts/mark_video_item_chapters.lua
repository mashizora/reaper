-- @desc    Mark Video Item Chapters
-- @author  mashizora
-- @version 0.1.0

-- check if ffprobe exists
if (reaper.ExecProcess('ffprobe -version', 0) == nil) then
    reaper.ShowMessageBox('ffprobe not found.', 'mark_video_item_chapters', 0)
    return
end
    ...
    if reaper.IsMediaItemSelected(item) then
        for take_idx = 0, (reaper.CountTakes(item) - 1) do
            ...
            if tonumber(code) == 0 then
                ...
                reaper.ShowMessageBox(message, 'mark_video_item_chapters', 0)
            end
